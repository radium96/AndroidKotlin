...
public class ArthemisFragment extends Fragment {

    private NPCViewModel viewModel;
    private FragmentArthemisBinding binding;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {

        int layoutId = R.layout.fragment_arthemis;
        binding = DataBindingUtil.inflate(inflater, layoutId, container, false);
        binding.setLifecycleOwner(this);

        View view = binding.getRoot();
        viewModel = new ViewModelProvider(this).get(NPCViewModel.class);
        viewModel.setList(0, 2);

        binding.arthemisRecy.setLayoutManager(new LinearLayoutManager(getActivity()));
        binding.arthemisRecy.setHasFixedSize(true);

        NPCAdapter mAdapter = new NPCAdapter(viewModel.getNPCList().getValue());
        binding.arthemisRecy.setAdapter(mAdapter);
